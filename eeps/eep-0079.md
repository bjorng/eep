...
FIXME: When no module name is given, this is called a *local* operation;
refererring to a record defined in the same module as the operation.

A creation done locally will always succeed. The record definition
in the same generation of the code will be used.

Other local record operations (update, matching, and field extraction)
never check whether the record is exported.
...
Anonymous operations follow a special rule. It is not possible 
to give a module name.

The special rule is that if the module name of the code executing the
operation is the same as the record name in the value, whether the
record is exported will not be checked. When the module names are not
the same, the operation can succeed only if the record value is marked
as exported.

FIXME: Automatic local or external?
...
This means that the definition of the record from the latest version
...
FIXME: old generation or old code version? Check this doc and Erlang documention.

For example, the record creation in the `foo/0` function will fail because
the `#bar{}` record is not exported:
...
FIXME: Write a general comment that the compiler cannot check things
in other modules. Other tools can.
...
FIXME: We could get a cycle of native record values.
...
FIXME: With module name. What happens without module name (what will
the compiler do).
...
* The native-record update expression references the field FN which is
  not defined (in the native-record definition).
...
FIXME: Use field names instead of keys.
FIXME: Note that default values are never a part of record value.

FIXME: Make field order more explicit.
...
FIXME: record/0 possible type clash.
