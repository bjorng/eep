...
based of an old code generation, even when executing "old" code.
...
To be decided: Should the following record creation succeed?
...
Currently it does. Specifying the current module name is exactly
the same omitting the module name.
...
* The native-record value's captured definition states it as
  ...
  *not exported* at the time of capture.
...
Prefixing a record with `?MODULE` is equivalent to omitting the module
name. That means `create3() -> #?MODULE:unknown_record{}.` is also an
