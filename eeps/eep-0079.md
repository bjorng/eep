...
When no module name is given, the operation is done locally,
...
If a module is given (either explictly or through an `-import_record()`),
the compiler will issue warnings for use of field names not present in
the definition.
...
* If the native-record definition is not visible at the call site (it
  is not exported), creation fails with a `{badrecord,Name}`
  exception.
